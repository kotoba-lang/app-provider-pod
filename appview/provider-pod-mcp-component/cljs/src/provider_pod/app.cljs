(ns provider-pod.app
  "provider-pod-mcp-component appview — reagent + re-frame, view built from
  ...
  The Svelte page never called the dispatcher or any provider-pod domain
  logic — it was pure static markup describing the surface's own metadata,
  and this port adds no provider/pod functionality that was not already
  there.

  The Svelte scaffold's `routes`/`vars` arrays were stale empty literals;
  this port corrects them from the component's actual `wrangler.jsonc`
  (`routes[].pattern` and the `vars` map keys), same as the landed
  reference this was copied from (`cloud-itonami/app-tia`'s
  `tia-mcp-component/cljs`).
  ...
  `../src/xrpc-proxy.ts` for its unmodified body and a note on why it is
...
(def default-db
  {:title "Provider Pod Mcp Component"
   :project "etzhayyim-project-provider-pod"
   :name "provider-pod-mcp-component"
   :kind "appview"
   :route-count 2
   :routes ["pht0hl9x.etzhayyim.com/*" "provider-pod.etzhayyim.com/*"]
   :vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_CAPABILITIES" "APP_DESCRIPTION"
          ...
          "APP_UI_TYPE"]
   :xrpc? true
   :relative-path "appview/provider-pod-mcp-component/cljs/src/provider_pod/app.cljs"})
